...
-- Teacher review of homework — a human read the actual text and judged it,
-- which is the feedback loop a pure grammar/fill-rate check can't give.
alter table homework add column if not exists teacher_status text not null default 'pending';
alter table homework add column if not exists teacher_comment text;
alter table homework add column if not exists reviewed_at timestamptz;
alter table homework drop constraint if exists homework_teacher_status_check;
alter table homework add constraint homework_teacher_status_check
